...
import cores.utils
import cores.float_utils
import cores.model_management_utils
import cores.lora
import cores.hooks
import cores.patcher_extension
from cores.patcher_extension import CallbacksMP, WrappersMP, PatcherInjection
from cores.types import UnetWrapperFunction
...
def create_model_options_clone(orig_model_options: dict):
    return cores.patcher_extension.copy_nested_dicts(orig_model_options)
...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [
            ...
            return cores.float_utils.stochastic_rounding(
                cores.lora.calculate_weight(
        ...
        return cores.lora.calculate_weight(
            self.patches[self.key],
            weight,
            self.key,
            intermediate_dtype=intermediate_dtype,
...
def get_key_weight(model, key):
    ...
    if len(op_keys) < 2:
        weight = cores.utils.get_attr(model, key)
    else:
        op = cores.utils.get_attr(model, op_keys[0])
        try:
...
class ModelPatcher:
    def __init__(
        ...
        self.hook_patches: dict[cores.hooks._HookRef] = {}
        self.hook_patches_backup: dict[cores.hooks._HookRef] = None
        self.hook_backup: dict[str, tuple[torch.Tensor, torch.device]] = {}
        self.cached_hook_patches: dict[
            cores.hooks.HookGroup, dict[str, torch.Tensor]
        ] = {}
        self.current_hooks: Optional[cores.hooks.HookGroup] = None
        self.forced_hooks: Optional[cores.hooks.HookGroup] = (
            None  # NOTE: only used for CLIP at this time
        )
        self.is_clip = False
        self.hook_mode = cores.hooks.EnumHookMode.MaxSpeed
    ...
    def model_size(self):
        if self.size > 0:
            return self.size
        self.size = cores.model_management_utils.module_size(self.model)
        return self.size
    ...
    def get_model_object(self, name: str) -> torch.nn.Module:
        ...
        if name in self.object_patches:
            ...
            if name in self.object_patches_backup:
                ...
                return cores.utils.get_attr(self.model, name)
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        if device_to is not None:
            temp_weight = cores.model_management_utils.cast_to_device(
        ...
        out_weight = cores.lora.calculate_weight(self.patches[key], temp_weight, key)
        if set_func is None:
            out_weight = cores.float_utils.stochastic_rounding(
                out_weight, weight.dtype, seed=string_to_seed(key)
            )
            if inplace_update:
                cores.utils.copy_to_param(self.model, key, out_weight)
            else:
                cores.utils.set_attr_param(self.model, key, out_weight)
        else:
            set_func(
    ...
    def _load_list(self):
        ...
        for n, m in self.model.named_modules():
            ...
            if not skip and (hasattr(m, "comfy_cast_weights") or len(params) > 0):
                loading.append(
                    (cores.model_management_utils.module_size(m), n, m, params)
                )
        return loading
            ...
            for x in load_completely:
                ...
                for param in params:
                    self.patch_weight_to_device(
                        "{}.{}".format(n, param), device_to=device_to
                    )
    ...
    def patch_model(
        ...
        with self.use_ejected():
            for k in self.object_patches:
                old = cores.utils.set_attr(self.model, k, self.object_patches[k])
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    cores.utils.copy_to_param(self.model, k, bk.weight)
                else:
                    cores.utils.set_attr_param(self.model, k, bk.weight)
        ...
        for k in keys:
            cores.utils.set_attr(self.model, k, self.object_patches_backup[k])
                ...
                if (
                    ...
                    for param in params:
                        ...
                        if bk is not None:
                            ...
                            if bk.inplace_update:
                                cores.utils.copy_to_param(self.model, key, bk.weight)
                            else:
                                cores.utils.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
                        ...
                        if lowvram_possible:
                            if weight_key in self.patches:
                                m.weight_function.append(
                                    LowVramPatch(weight_key, self.patches)
                                )
                                patch_counter += 1
                            if bias_key in self.patches:
                                m.bias_function.append(
                                    LowVramPatch(bias_key, self.patches)
                                )
                                patch_counter += 1
                            cast_weight = True
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        logging.warning(
            "The ModelPatcher.calculate_weight function is deprecated, please use: cores.lora.calculate_weight instead"
        )
        return cores.lora.calculate_weight(
    ...
    def set_hook_mode(self, hook_mode: cores.hooks.EnumHookMode):
        self.hook_mode = hook_mode

    def prepare_hook_patches_current_keyframe(
        self,
        t: torch.Tensor,
        hook_group: cores.hooks.HookGroup,
    ...
    def register_all_hook_patches(
        self,
        hooks: cores.hooks.HookGroup,
        target_dict: dict[str],
        model_options: dict = None,
        registered: cores.hooks.HookGroup = None,
    ):
        self.restore_hook_patches()
        if registered is None:
            registered = cores.hooks.HookGroup()
        # handle WeightHooks
        weight_hooks_to_register: list[cores.hooks.WeightHook] = []
        for hook in hooks.get_type(cores.hooks.EnumHookType.Weight):
    ...
    def get_combined_hook_patches(self, hooks: cores.hooks.HookGroup):
    ...
    def apply_hooks(
        ...
        if self.current_hooks == hooks and (
            ...
            return cores.hooks.create_transformer_options_from_hooks(
    ...
    def patch_hooks(self, hooks: cores.hooks.HookGroup):
        with self.use_ejected():
            if hooks is not None:
                model_sd_keys = list(self.model_state_dict().keys())
                memory_counter = None
                if self.hook_mode == cores.hooks.EnumHookMode.MaxSpeed:
                    # TODO: minimum_counter should have a minimum that conforms to loaded model requirements
                    memory_counter = MemoryCounter(
                        initial=cores.model_management_utils.get_free_memory(
                            self.load_device
                        ),
                        minimum=cores.model_management_utils.minimum_inference_memory()
                        * 2,
                    )
    ...
    def patch_cached_hook_weights(
        ...
        if key not in self.hook_backup:
            weight: torch.Tensor = cores.utils.get_attr(self.model, key)
    ...
    def patch_hook_weight_to_device(
        ...
        out_weight = cores.lora.calculate_weight(
