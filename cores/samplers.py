...
from cores.k_diffusion import sampling as k_diffusion_sampling
from cores.extra_samplers import uni_pc
from typing import TYPE_CHECKING, Callable, NamedTuple

if TYPE_CHECKING:
    from cores.model_patcher import ModelPatcher
    from cores.model_base import BaseModel
    from cores.controlnet import ControlBase
import torch
...
def finalize_default_conds(
    model: "BaseModel",
    hooked_to_run: dict[cores.hooks.HookGroup, list[tuple[tuple, int]]],
...
def calc_cond_batch(
    ...
    executor = cores.patcher_extension.WrapperExecutor.new_executor(
        _calc_cond_batch,
        cores.patcher_extension.get_all_wrappers(
            cores.patcher_extension.WrappersMP.CALC_COND_BATCH,
            model_options,
...
def _calc_cond_batch(
    ...
    for hooks, to_run in hooked_to_run.items():
        while len(to_run) > 0:
            ...
            free_memory = model_management_utils.get_free_memory(x_in.device)
            ...
            if "transformer_options" in model_options:
                transformer_options = cores.patcher_extension.merge_nested_dicts(
                    transformer_options,
...
def calc_cond_uncond_batch(
    ...
    logging.warning(
        "WARNING: The cores.samplers.calc_cond_uncond_batch function is deprecated please use the calc_cond_batch one instead."
    )
...
def resolve_areas_and_cond_masks(conditions, h, w, device):
    logging.warning(
        "WARNING: The cores.samplers.resolve_areas_and_cond_masks function is deprecated please use the resolve_areas_and_cond_masks_multidim one instead."
    )
...
def preprocess_conds_hooks(conds: dict[str, list[dict[str]]]):
    ...
    hook_replacement: dict[tuple[ControlBase, cores.hooks.HookGroup], list[dict]] = {}
    for k in conds:
        for kk in conds[k]:
            if "control" in kk:
                ...
                if len(extra_hooks) > 0:
                    hooks: cores.hooks.HookGroup = kk.get("hooks", None)
    ...
    for key, conds_to_modify in hook_replacement.items():
        control = key[0]
        hooks = key[1]
        hooks = cores.hooks.HookGroup.combine_all_hooks(
...
def filter_registered_hooks_on_conds(
    ...
    registered: cores.hooks.HookGroup = model_options.get("registered_hooks", None)
    ...
    hook_replacement: dict[cores.hooks.HookGroup, list[dict]] = {}
    for k in conds:
        for kk in conds[k]:
            hooks: cores.hooks.HookGroup = kk.get("hooks", None)
            if hooks is not None:
...
class CFGGuider:
    ...
    def inner_set_conds(self, conds):
        for k in conds:
            self.original_conds[k] = cores.sampler_helpers.convert_cond(conds[k])
    ...
    def inner_sample(
        ...
        extra_model_options = cores.model_patcher.create_model_options_clone(
            self.model_options
        )
        ...
        executor = cores.patcher_extension.WrapperExecutor.new_class_executor(
            sampler.sample,
            sampler,
            cores.patcher_extension.get_all_wrappers(
                cores.patcher_extension.WrappersMP.SAMPLER_SAMPLE,
    ...
    def outer_sample(
        ...
        self.inner_model, self.conds, self.loaded_models = (
            cores.sampler_helpers.prepare_sampling(
        ...
        if denoise_mask is not None:
            denoise_mask = cores.sampler_helpers.prepare_mask(
        ...
        cores.sampler_helpers.cleanup_models(self.conds, self.loaded_models)
    ...
    def sample(
        ...
        try:
            ...
            self.model_options = cores.model_patcher.create_model_options_clone(
                self.model_options
            )
            # if one hook type (or just None), then don't bother caching weights for hooks (will never change after first step)
            orig_hook_mode = self.model_patcher.hook_mode
            if get_total_hook_groups_in_conds(self.conds) <= 1:
                self.model_patcher.hook_mode = cores.hooks.EnumHookMode.MinVram
            cores.sampler_helpers.prepare_model_patcher(
            ...
            executor = cores.patcher_extension.WrapperExecutor.new_class_executor(
                ...
                self,
                cores.patcher_extension.get_all_wrappers(
                    cores.patcher_extension.WrappersMP.OUTER_SAMPLE,
                    self.model_options,
                    is_model_options=True,
                ),
            )
