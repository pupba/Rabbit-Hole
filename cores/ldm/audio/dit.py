...
class FourierFeatures(nn.Module):
    ...
    def forward(self, input):
        f = 2 * math.pi * input @ cores.ops.cast_to_input(self.weight.T, input)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        beta = self.beta
        if beta is not None:
            beta = cores.ops.cast_to_input(beta, x)
        return F.layer_norm(
            x, x.shape[-1:], weight=cores.ops.cast_to_input(self.gamma, x), bias=beta
        )
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum(
            "i , j -> i j", t, cores.ops.cast_to_input(self.inv_freq, t)
        )
        ...
        scale = cores.ops.cast_to_input(self.scale, t) ** rearrange(power, "n -> n 1")
...
class ContinuousTransformer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        patches_replace = kwargs.get("transformer_options", {}).get(
            "patches_replace", {}
        )
