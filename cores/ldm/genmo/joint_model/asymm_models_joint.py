...
from cores.ldm.modules.attention import optimized_attention

from cores.ldm.genmo.joint_model.layers import (
    FeedForward,
    PatchEmbed,
    TimestepEmbedder,
)

from cores.ldm.genmo.joint_model.rope_mixed import (
    compute_mixed_rotation,
    create_position_matrix,
)
from cores.ldm.genmo.joint_model.temporal_rope import apply_rotary_emb_qk_real
from cores.ldm.genmo.joint_model.utils import (
    AttentionPool,
    modulate,
)

import cores.ldm.common_dit
import cores.ops


def modulated_rmsnorm(x, scale, eps=1e-6):
    # Normalize and modulate
    x_normed = cores.ldm.common_dit.rms_norm(x, eps=eps)
...
def residual_tanh_gated_rmsnorm(x, x_res, gate, eps=1e-6):
    ...
    x_normed = cores.ldm.common_dit.rms_norm(x_res, eps=eps) * tanh_gate
