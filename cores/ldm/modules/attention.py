...
from cores.ldm.modules.diffusionmodules.util import AlphaBlender, timestep_embedding
from cores.ldm.modules.sub_quadratic_attention import efficient_dot_product_attention

from cores import model_management_utils

if model_management_utils.xformers_enabled():
    import xformers
    import xformers.ops

if model_management_utils.sage_attention_enabled():
    try:
...
if model_management_utils.flash_attention_enabled():
    try:
...
ops = cores.ops.disable_weight_init

FORCE_UPCAST_ATTENTION_DTYPE = model_management_utils.force_upcast_attention_dtype()
...
def attention_sub_quad(
    ...
    mem_free_total, _ = model_management_utils.get_free_memory(query.device, True)
...
def attention_split(
    ...
    mem_free_total = model_management_utils.get_free_memory(q.device)
    ...
    while True:
        try:
            ...
            for i in range(0, q.shape[1], slice_size):
                ...
                del s2
            break
        except model_management_utils.OOM_EXCEPTION as e:
            if first_op_done == False:
                model_management_utils.soft_empty_cache(True)
...
if model_management_utils.is_nvidia():  # pytorch 2.3 and up seem to have this issue.
...
elif model_management_utils.xformers_enabled():
    logging.info("Using xformers attention")
    optimized_attention = attention_xformers
elif model_management_utils.flash_attention_enabled():
    logging.info("Using Flash Attention")
    optimized_attention = attention_flash
elif model_management_utils.pytorch_attention_enabled():
...
def optimized_attention_for_device(device, mask=False, small_input=False):
    if small_input:
        if model_management_utils.pytorch_attention_enabled():
