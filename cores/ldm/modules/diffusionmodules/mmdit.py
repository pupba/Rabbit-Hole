...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if self.dynamic_img_pad:
            x = cores.ldm.common_dit.pad_to_patch_size(
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x):
        return cores.ldm.common_dit.rms_norm(x, self.weight, self.eps)
...
class MMDiT(nn.Module):
            ...
            if ("double_block", i) in blocks_replace:
                ...
                def block_wrap(args):
                    ...
                    out["txt"], out["img"] = self.joint_blocks[i](
                        args["txt"], args["img"], c=args["vec"]
                    )
                    return out

                out = blocks_replace[("double_block", i)](
                    {"img": x, "txt": context, "vec": c_mod},
    ...
    def forward(
        ...
        hw = x.shape[-2:]
        x = self.x_embedder(x) + cores.ops.cast_to_input(
