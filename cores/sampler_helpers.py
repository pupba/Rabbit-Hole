...
def prepare_mask(noise_mask, shape, device):
    return cores.utils.reshape_mask(noise_mask, shape).to(device)
...
def get_hooks_from_cond(cond, full_hooks: cores.hooks.HookGroup):
    ...
    extra_hooks = cores.hooks.HookGroup.combine_all_hooks(hooks_list)
...
def get_additional_models_from_model_options(model_options: dict[str] = None):
    ...
    if model_options is not None and "registered_hooks" in model_options:
        registered: cores.hooks.HookGroup = model_options["registered_hooks"]
        for hook in registered.get_type(cores.hooks.EnumHookType.AdditionalModels):
            hook: cores.hooks.AdditionalModelsHook
...
def prepare_sampling(model: ModelPatcher, noise_shape, conds, model_options=None):
    executor = cores.patcher_extension.WrapperExecutor.new_executor(
        ...
        cores.patcher_extension.get_all_wrappers(
            cores.patcher_extension.WrappersMP.PREPARE_SAMPLING,
            model_options,
...
def _prepare_sampling(model: ModelPatcher, noise_shape, conds, model_options=None):
    ...
    cores.model_management_utils.load_models_gpu(
        [model] + models,
...
def prepare_model_patcher(model: "ModelPatcher", conds, model_options: dict):
    ...
    model_options["transformer_options"]["wrappers"] = (
        cores.patcher_extension.copy_nested_dicts(model.wrappers)
    )
    model_options["transformer_options"]["callbacks"] = (
        cores.patcher_extension.copy_nested_dicts(model.callbacks)
    )
    # begin registering hooks
    registered = cores.hooks.HookGroup()
    target_dict = cores.hooks.create_target_dict(cores.hooks.EnumWeightTarget.Model)
    # handle all TransformerOptionsHooks
    for hook in hooks.get_type(cores.hooks.EnumHookType.TransformerOptions):
        hook: cores.hooks.TransformerOptionsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    # handle all AddModelsHooks
    for hook in hooks.get_type(cores.hooks.EnumHookType.AdditionalModels):
        hook: cores.hooks.AdditionalModelsHook
        hook.add_hook_patches(model, model_options, target_dict, registered)
    ...
    for wc_name in ["wrappers", "callbacks"]:
        cores.patcher_extension.merge_nested_dicts(
