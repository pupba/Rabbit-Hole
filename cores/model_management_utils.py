...
try:
    ...
    xpu_available = (
        torch_version_numeric[0] < 2
        or (torch_version_numeric[0] == 2 and torch_version_numeric[1] <= 4)
...
if args.directml is not None:
    # import torch_directml  # AMD, Intel ARM GPU, In this project not support yet
...
# try:
#     import intel_extension_for_pytorch as ipex  # Intel CPU this project not support yet
...
try:
    if is_amd():
        ...
        if (
            ...
            if (
                torch_version_numeric[0] >= 2 and torch_version_numeric[1] >= 7
            ):  # works on 2.6 but doesn't actually seem to improve much
                if any(
                    (a in arch) for a in ["gfx1100", "gfx1101"]
                ):  # TODO: more arches
                    ENABLE_PYTORCH_ATTENTION = True
except:
    pass
