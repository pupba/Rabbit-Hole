...
from cores.k_diffusion import utils
from cores.k_diffusion import deis
import cores.model_patcher
import cores.model_sampling
...
def sample_euler_ancestral(
    ...
    if isinstance(
        model.inner_model.inner_model.model_sampling, cores.model_sampling.CONST
    ):
...
def sample_euler_cfg_pp(model, x, sigmas, extra_args=None, callback=None, disable=None):
    ...
    extra_args["model_options"] = (
        cores.model_patcher.set_model_options_post_cfg_function(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            if inject_noise:
                ...
                noise_coeff_2 = (
                    (-2 * r * h * eta).expm1() - (-2 * h * eta).expm1()
                ).sqrt()
                noise_1, noise_2 = noise_sampler(sigmas[i], sigma_s), noise_sampler(
                    sigma_s, sigmas[i + 1]
                )
            ...
            if inject_noise:
                x = (
                    x
                    + sigmas[i + 1]
                    * (noise_coeff_2 * noise_1 + noise_coeff_1 * noise_2)
                    * s_noise
                )
    return x
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if sigmas[i + 1] == 0:
            ...
            if inject_noise:
                ...
                noise_coeff_2 = (
                    (-2 * r_1 * h * eta).expm1() - (-2 * r_2 * h * eta).expm1()
                ).sqrt()
                noise_coeff_3 = (
                    (-2 * r_2 * h * eta).expm1() - (-2 * h * eta).expm1()
                ).sqrt()
            ...
            x_3 = (
                (coeff_2 + 1) * x
                - coeff_2 * denoised
                + (r_2 / r_1) * (coeff_2 / (r_2 * h_eta) + 1) * (denoised_2 - denoised)
            )
            if inject_noise:
                x_3 = (
                    x_3
            ...
            x = (
                (coeff_3 + 1) * x
                - coeff_3 * denoised
                + (1.0 / r_2) * (coeff_3 / h_eta + 1) * (denoised_3 - denoised)
            )
            if inject_noise:
                x = (
                    x
                    + sigmas[i + 1]
                    * (
                        noise_coeff_3 * noise_1
                        + noise_coeff_2 * noise_2
                        + noise_coeff_1 * noise_3
                    )
                    * s_noise
                )
    return x
