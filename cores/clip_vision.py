from cores.utils import load_torch_file, transformers_convert, state_dict_prefix_replace
import os
import torch
import json
import logging
...
import cores.utils
import cores.clip_model_utils
import cores.image_encoders.dino2
...
IMAGE_ENCODERS = {
    "clip_vision_model": cores.clip_model_utils.CLIPVisionModelProjection,
    "siglip_vision_model": cores.clip_model_utils.CLIPVisionModelProjection,
    "dinov2": cores.image_encoders.dino2.Dinov2Model,
}
...
class ClipVisionModel:
    ...
    def encode_image(self, image, crop=True):
        cores.model_management_utils.load_model_gpu(self.patcher)
        pixel_values = clip_preprocess(
            image.to_clip_model_utils(self.load_device),
        ...
        outputs["last_hidden_state"] = out[0].to(
            cores.model_management_utils.intermediate_device()
        )
        outputs["image__clip_model_utilsembeds"] = out[2].to(
            cores.model_management_utils.intermediate_device()
        )
        outputs["penult_clip_model_utilsimate_hidden_states"] = out[1].to(
            cores.model_management_utils.intermediate_device()
        )
        outputs["mm_pro_clip_model_utilsjected"] = out[3]
        return outputs
