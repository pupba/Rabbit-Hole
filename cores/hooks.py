...
if TYPE_CHECKING:
    from cores.model_patcher import ModelPatcher, PatcherInjection
    from cores.model_base import BaseModel
    from cores.sd_model import CLIP
import cores.lora
...
class WeightHook(Hook):
    ...
    def add_hook_patches(
        ...
        if self.need_weight_init:
            key_map = {}
            if target == EnumWeightTarget.Clip:
                key_map = cores.lora.model_lora_keys_clip(model.model, key_map)
            else:
                key_map = cores.lora.model_lora_keys_unet(model.model, key_map)
            weights = cores.lora.load_lora(self.weights, key_map, log_missing=False)
        else:
            if target == EnumWeightTarget.Clip:
...
def load_hook_lora_for_models(
    ...
    hook = WeightHook()
    hook_group.add(hook)
    loaded: dict[str] = cores.lora.load_lora(lora, key_map)
    if model is not None:
