...
import cores.weight_adapter as weight_adapter
import logging
import torch
...
def model_lora_keys_unet(model, key_map={}):
    ...
    diffusers_keys = cores.utils.unet_to_diffusers(model.model_config.unet_config)
    ...
    if isinstance(model, cores.model_base.StableCascade_C):
        for k in sdk:
            if k.startswith("diffusion_model."):
                if k.endswith(".weight"):
                    key_lora = k[len("diffusion_model.") : -len(".weight")].replace(
    ...
    if isinstance(model, cores.model_base.SD3):  # Diffusers lora SD3
        diffusers_keys = cores.utils.mmdit_to_diffusers(
    ...
    if isinstance(model, cores.model_base.AuraFlow):  # Diffusers lora AuraFlow
        diffusers_keys = cores.utils.auraflow_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
    ...
    if isinstance(model, cores.model_base.PixArt):
        diffusers_keys = cores.utils.pixart_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
    ...
    if isinstance(model, cores.model_base.HunyuanDiT):
        for k in sdk:
    ...
    if isinstance(model, cores.model_base.Flux):  # Diffusers lora Flux
        diffusers_keys = cores.utils.flux_to_diffusers(
            model.model_config.unet_config, output_prefix="diffusion_model."
        )
        for k in diffusers_keys:
            if k.endswith(".weight"):
    ...
    if isinstance(model, cores.model_base.GenmoMochi):
        for k in sdk:
    ...
    if isinstance(model, cores.model_base.HunyuanVideo):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
    ...
    if isinstance(model, cores.model_base.HiDream):
        for k in sdk:
            if k.startswith("diffusion_model."):
                if k.endswith(".weight"):
                    key_lora = k[len("diffusion_model.") : -len(".weight")].replace(
...
def calculate_weight(
    ...
    for p in patches:
        ...
        if isinstance(v, list):
            v = (
                calculate_weight(
                    v[1:],
                    v[0][1](
                        cores.model_management_utils.cast_to_device(
        ...
        if patch_type == "diff":
            ...
            diff_weight = cores.model_management_utils.cast_to_device(
