import json
import cores.supported_models_utils
import cores.supported_models_base_utils
import cores.utils
import math
import logging
import torch
...
def detect_unet_config(state_dict, key_prefix, metadata=None):
    ...
    if (
        ...
        dit_config["in_channels"] = state_dict[
            "{}img_in.proj.weight".format(key_prefix)
        ].shape[
            1
...
def model_config_from_unet_config(unet_config, state_dict=None):
    for model_config in cores.supported_models_utils.models:
...
def model_config_from_unet(
    ...
    if model_config is None and use_base_if_no_match:
        model_config = cores.supported_models_base_utils.BASE(unet_config)
...
def unet_config_from_diffusers_unet(state_dict, dtype=None):
    ...
    for unet_config in supported_models_utils:
        matches = True
        for k in match:
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if "joint_transformer_blocks.0.attn.add_k_proj.weight" in state_dict:  # AuraFlow
        ...
        sd_map = cores.utils.auraflow_to_diffusers(
        ...
        sd_map = cores.utils.pixart_to_diffusers(
        ...
        sd_map = cores.utils.flux_to_diffusers(
            {
                "depth": depth,
        ...
        sd_map = cores.utils.mmdit_to_diffusers(
