...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return cores.ldm.common_dit.rms_norm(x, w, self.eps)
...
class Attention(nn.Module):
    def __init__(self, config: Llama2Config, device=None, dtype=None, ops: Any = None):
        ...
        self.q_proj = ops.Linear(
            config.hidden_size, self.inner_size, bias=False, device=device, dtype=dtype
        )
...
class Llama2_(nn.Module):
    ...
    def forward(
        ...
        freqs_cis = precompute_freqs_cis(
            self.config.head_dim, x.shape[1], self.config.rope_theta, device=x.device
        )
