...
class ClipTokenWeightEncoder:
    def encode_token_weights(self, token_weight_pairs):
        ...
        if pooled is not None:
            first_pooled = pooled[0:1].to(model_management_utils.intermediate_device())
        else:
            first_pooled = pooled
            ...
            for k in o[2]:
                ...
                if k == "attention_mask":
                    v = (
                        ...
                        .flatten()
                        .unsqueeze(dim=0)
                        .to(model_management_utils.intermediate_device())
                    )
                extra[k] = v
...
class SDClipModel(torch.nn.Module, ClipTokenWeightEncoder):
    ...
    def __init__(
        ...
        dtype=None,
        model_class=cores.clip_model_utils.CLIPTextModel,
        ...
        if operations is None:
            ...
            if scaled_fp8 is not None:
                operations = cores.ops.scaled_fp8_ops(
                    fp8_matrix_mult=False, override_dtype=scaled_fp8
                )
            else:
                operations = cores.ops.manual_cast
...
class SD1ClipModel(torch.nn.Module):
    def __init__(
        ...
        model_options={},
        clip_name="l",
        clip_model_utils=SD1CheckpointClipModel,
        name=None,
        **kwargs,
    ):
        super().__init__()
        ...
        clip_model_utils = model_options.get(
            "{}_class".format(self.clip), clip_model_utils
        )
