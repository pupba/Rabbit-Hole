from cores.text_encoder_utils import hunyuan_video
from cores.text_encoder_utils import sd3_clip
...
class HiDreamTEModel(torch.nn.Module):
    def __init__(
        ...
        if t5:
            dtype_t5 = cores.model_management_utils.pick_weight_dtype(
        ...
        if llama:
            dtype_llama = cores.model_management_utils.pick_weight_dtype(
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros(
                    (1, 768), device=cores.model_management_utils.intermediate_device()
                )
            ...
            if self.clip_g is not None:
                ...
                g_pooled = torch.zeros(
                    (1, 1280), device=cores.model_management_utils.intermediate_device()
                )
        ...
        if t5_out is None:
            t5_out = torch.zeros(
                (1, 128, 4096),
                device=cores.model_management_utils.intermediate_device(),
            )
