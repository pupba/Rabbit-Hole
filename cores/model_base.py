...
import torch
import logging
from cores.ldm.modules.diffusionmodules.openaimodel import UNetModel, Timestep
from cores.ldm.cascade.stage_c import StageC
from cores.ldm.cascade.stage_b import StageB
from cores.ldm.modules.encoders.noise_aug_modules import CLIPEmbeddingNoiseAugmentation
from cores.ldm.modules.diffusionmodules.upscaling import (
...
import cores.ldm.hydit.models
import cores.ldm.audio.dit
import cores.ldm.audio.embedders
import cores.ldm.flux.model
import cores.ldm.lightricks.model
import cores.ldm.hunyuan_video.model
import cores.ldm.cosmos.model
import cores.ldm.lumina.model
import cores.ldm.wan.model
import cores.ldm.hunyuan3d.model
import cores.ldm.hidream.model
import cores.ldm.chroma.model
import cores.ldm.ace.model

import cores.model_management_utils
import cores.patcher_extension
import cores.conds
import cores.ops
from enum import Enum
from cores import utils
import cores.latent_formats
import math
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from cores.model_patcher import ModelPatcher
...
from cores.model_sampling import (
    EPS,
    V_PREDICTION,
    EDM,
...
def model_sampling(model_config, model_type):
    ...
    if model_type == ModelType.EPS:
        ...
        c = cores.model_sampling.CONST
        s = cores.model_sampling.ModelSamplingDiscreteFlow
        ...
        c = cores.model_sampling.CONST
        s = cores.model_sampling.ModelSamplingFlux
    elif model_type == ModelType.IMG_TO_IMG:
        c = cores.model_sampling.IMG_TO_IMG
...
class BaseModel(torch.nn.Module):
    def __init__(
        ...
        if not unet_config.get("disable_unet_model_creation", False):
            if model_config.custom_operations is None:
                ...
                operations = cores.ops.pick_operations(
            ...
            if cores.model_management_utils.force_channels_last():
    ...
    def apply_model(
        ...
        **kwargs
        ...
        return cores.patcher_extension.WrapperExecutor.new_class_executor(
            self._apply_model,
            self,
            cores.patcher_extension.get_all_wrappers(
                cores.patcher_extension.WrappersMP.APPLY_MODEL, transformer_options
            ),
    ...
    def concat_cond(self, **kwargs):
        if len(self.concat_keys) > 0:
            ...
            if concat_latent_image.shape[1:] != noise.shape[1:]:
                concat_latent_image = cores.common_upscale(
                    concat_latent_image,
                    noise.shape[-1],
                    noise.shape[-2],
                    "bilinear",
            ...
            concat_latent_image = cores.resize_to_batch_size(
            ...
            if denoise_mask is not None:
                ...
                if denoise_mask.shape[-2:] != noise.shape[-2:]:
                    denoise_mask = cores.common_upscale(
                        denoise_mask,
                        noise.shape[-1],
                        noise.shape[-2],
                        "bilinear",
                ...
                denoise_mask = cores.resize_to_batch_size(
    ...
    def extra_conds(self, **kwargs):
        ...
        if concat_cond is not None:
            out["c_concat"] = cores.conds.CONDNoiseShape(concat_cond)

        adm = self.encode_adm(**kwargs)
        if adm is not None:
            out["y"] = cores.conds.CONDRegular(adm)

        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out["c_crossattn"] = cores.conds.CONDCrossAttn(cross_attn)

        cross_attn_cnet = kwargs.get("cross_attn_controlnet", None)
        if cross_attn_cnet is not None:
            out["crossattn_controlnet"] = cores.conds.CONDCrossAttn(cross_attn_cnet)

        c_concat = kwargs.get("noise_concat", None)
        if c_concat is not None:
            out["c_concat"] = cores.conds.CONDNoiseShape(c_concat)
    ...
    def memory_required(self, input_shape):
        if (
            cores.model_management_utils.xformers_enabled()
            or cores.model_management_utils.pytorch_attention_flash_attention()
        ):
            dtype = self.get_dtype()
            if self.manual_cast_dtype is not None:
                dtype = self.manual_cast_dtype
            ...
            return (
                area
                * cores.model_management_utils.dtype_size(dtype)
                * 0.01
...
class SVD_img2vid(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if latent_image.shape[1:] != noise.shape[1:]:
            latent_image = cores.common_upscale(
                latent_image, noise.shape[-1], noise.shape[-2], "bilinear", "center"
            )

        latent_image = cores.resize_to_batch_size(latent_image, noise.shape[0])

        out["c_concat"] = cores.conds.CONDNoiseShape(latent_image)
        ...
        if "time_conditioning" in kwargs:
            out["time_context"] = cores.conds.CONDCrossAttn(kwargs["time_conditioning"])

        out["num_video_frames"] = cores.conds.CONDConstant(noise.shape[0])
        return out
...
class SV3D_p(SVD_img2vid):
    ...
    def encode_adm(self, **kwargs):
        ...
        out = list(map(lambda a: cores.resize_to_batch_size(a, noise.shape[0]), out))
...
class Stable_Zero123(BaseModel):
    def __init__(
        ...
        super().__init__(model_config, model_type, device=device)
        self.cc_projection = cores.ops.manual_cast.Linear(
...
class SD_X4Upscaler(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        out["c_concat"] = cores.conds.CONDNoiseShape(image)
        out["y"] = cores.conds.CONDRegular(noise_level)
    ...
    def __init__(self, model_config, model_type=ModelType.EPS, device=None):
        ...
        if model_type == ModelType.V_PREDICTION_EDM:
            self.process_ip2p_image_in = (
                lambda image: cores.latent_formats.SDXL().process_in(image)
            )  # cosxl ip2p
        else:
...
class Lotus(BaseModel):
    def extra_conds(self, **kwargs):
        ...
        out["y"] = cores.conds.CONDRegular(task_emb)
        return out
...
class StableCascade_C(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if clip_text_pooled is not None:
            out["clip_text_pooled"] = cores.conds.CONDRegular(clip_text_pooled)
        ...
        out["clip_img"] = cores.conds.CONDRegular(clip_img)
        out["sca"] = cores.conds.CONDRegular(torch.zeros((1,)))
        out["crp"] = cores.conds.CONDRegular(torch.zeros((1,)))

        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out["clip_text"] = cores.conds.CONDCrossAttn(cross_attn)
        return out
...
class StableCascade_B(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        clip_text_pooled = kwargs["pooled_output"]
        if clip_text_pooled is not None:
            out["clip"] = cores.conds.CONDRegular(clip_text_pooled)
        ...
        out["effnet"] = cores.conds.CONDRegular(prior)
        out["sca"] = cores.conds.CONDRegular(torch.zeros((1,)))
        return out
...
class SD3(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out["c_crossattn"] = cores.conds.CONDRegular(cross_attn)
        return out
...
class AuraFlow(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.aura.mmdit.MMDiT,
        )
...
class StableAudio1(BaseModel):
    def __init__(
        ...
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.audio.dit.AudioDiffusionTransformer,
        )
        self.seconds_start_embedder = cores.ldm.audio.embedders.NumberConditioner(
            768, min_val=0, max_val=512
        )
        self.seconds_total_embedder = cores.ldm.audio.embedders.NumberConditioner(
            768, min_val=0, max_val=512
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        out["global_embed"] = cores.conds.CONDRegular(global_embed)
...
class HunyuanDiT(BaseModel):
    def __init__(self, model_config, model_type=ModelType.V_PREDICTION, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.hydit.models.HunYuanDiT,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        out["image_meta_size"] = cores.conds.CONDRegular(
...
class PixArt(BaseModel):
    def __init__(self, model_config, model_type=ModelType.EPS, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.pixart.pixartms.PixArtMS,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if width is not None and height is not None:
            out["c_size"] = cores.conds.CONDRegular(
                torch.FloatTensor([[height, width]])
            )
            out["c_ar"] = cores.conds.CONDRegular(
...
class Flux(BaseModel):
    def __init__(
        ...
        device=None,
        unet_model=cores.ldm.flux.model.Flux,
    ):
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            ...
            attention_mask = cores.upscale_dit_mask(
                attention_mask, mask_ref_size, (h_tok, w_tok)
            )
            out["attention_mask"] = cores.conds.CONDRegular(attention_mask)

        guidance = kwargs.get("guidance", 3.5)
        if guidance is not None:
            out["guidance"] = cores.conds.CONDRegular(torch.FloatTensor([guidance]))
        return out
...
class GenmoMochi(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.genmo.joint_model.asymm_models_joint.AsymmDiTJoint,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            out["attention_mask"] = cores.conds.CONDRegular(attention_mask)
            out["num_tokens"] = cores.conds.CONDConstant(
...
class LTXV(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.lightricks.model.LTXVModel,
        )  # TODO
    ...
    def extra_conds(self, **kwargs):
        ...
        out["frame_rate"] = cores.conds.CONDConstant(kwargs.get("frame_rate", 25))

        denoise_mask = kwargs.get("concat_mask", kwargs.get("denoise_mask", None))
        if denoise_mask is not None:
            out["denoise_mask"] = cores.conds.CONDRegular(denoise_mask)

        keyframe_idxs = kwargs.get("keyframe_idxs", None)
        if keyframe_idxs is not None:
            out["keyframe_idxs"] = cores.conds.CONDRegular(keyframe_idxs)
...
class HunyuanVideo(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.hunyuan_video.model.HunyuanVideo,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if guiding_frame_index is not None:
            out["guiding_frame_index"] = cores.conds.CONDRegular(
...
class CosmosVideo(BaseModel):
    def __init__(
        ...
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.cosmos.model.GeneralDIT,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        out["fps"] = cores.conds.CONDConstant(kwargs.get("frame_rate", None))
        return out
...
class Lumina2(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.lumina.model.NextDiT,
        )
...
class WAN21(BaseModel):
    def __init__(
        ...
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.wan.model.WanModel,
        )
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        if clip_vision_output is not None:
            out["clip_fea"] = cores.conds.CONDRegular(
...
class WAN21_Vace(WAN21):
    def __init__(
        ...
        super(WAN21, self).__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.wan.model.VaceWanModel,
        )
        self.image_to_video = image_to_video
    ...
    def extra_conds(self, **kwargs):
        ...
        out["vace_context"] = cores.conds.CONDRegular(
            torch.cat([vace_frames.to(noise), mask.to(noise)], dim=1)
        )

        vace_strength = kwargs.get("vace_strength", 1.0)
        out["vace_strength"] = cores.conds.CONDConstant(vace_strength)
        return out
...
class Hunyuan3Dv2(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.hunyuan3d.model.Hunyuan3Dv2,
        )
...
class HiDream(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.hidream.model.HiDreamImageTransformer2DModel,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if conditioning_llama3 is not None:
            out["encoder_hidden_states_llama3"] = cores.conds.CONDRegular(
                conditioning_llama3
            )
        image_cond = kwargs.get("concat_latent_image", None)
        if image_cond is not None:
            out["image_cond"] = cores.conds.CONDNoiseShape(
...
class Chroma(Flux):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.chroma.model.Chroma,
        )
...
class ACEStep(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(
            ...
            model_type,
            device=device,
            unet_model=cores.ldm.ace.model.ACEStepTransformer2DModel,
        )
    ...
    def extra_conds(self, **kwargs):
        ...
        if cross_attn is not None:
            out["lyric_token_idx"] = cores.conds.CONDRegular(conditioning_lyrics)
        out["speaker_embeds"] = cores.conds.CONDRegular(
            torch.zeros(noise.shape[0], 512, device=noise.device, dtype=noise.dtype)
        )
        out["lyrics_strength"] = cores.conds.CONDConstant(
