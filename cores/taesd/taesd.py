...
def conv(n_in, n_out, **kwargs):
    return cores.ops.disable_weight_init.Conv2d(n_in, n_out, 3, padding=1, **kwargs)
...
class Block(nn.Module):
    def __init__(self, n_in, n_out):
        ...
        self.skip = (
            cores.ops.disable_weight_init.Conv2d(n_in, n_out, 1, bias=False)
            if n_in != n_out
            else nn.Identity()
        )
        self.fuse = nn.ReLU()
    ...
    def __init__(self, encoder_path=None, decoder_path=None, latent_channels=4):
        ...
        if encoder_path is not None:
            self.taesd_encoder.load_state_dict(
                cores.utils.load_torch_file(encoder_path, safe_load=True)
            )
        if decoder_path is not None:
            self.taesd_decoder.load_state_dict(
                cores.utils.load_torch_file(decoder_path, safe_load=True)
            )
