...
import math
import struct
import cores.checkpoint_pickle
...
def load_torch_file(ckpt, safe_load=False, device=None, return_metadata=False):
    ...
    if ckpt.lower().endswith(".safetensors") or ckpt.lower().endswith(".sft"):
        ...
        if safe_load or ALWAYS_SAFE_LOAD:
            ...
            pl_sd = torch.load(
                ckpt, map_location=device, pickle_module=cores.checkpoint_pickle
            )
...
def get_attr(obj, attr: str):
    ...
    Important:
        Always prefer `utils.model_patcher.ModelPatcher.get_model_object` when
