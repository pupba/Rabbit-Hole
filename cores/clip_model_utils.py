...
class CLIPEmbeddings(torch.nn.Module):
    ...
    def forward(self, input_tokens, dtype=torch.float32):
        return self.token_embedding(input_tokens, out_dtype=dtype) + cores.ops.cast_to(
...
class CLIPVisionEmbeddings(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        if self.class_embedding is not None:
            embeds = torch.cat(
                [
                    cores.ops.cast_to_input(self.class_embedding, embeds).expand(
        ...
        return embeds + cores.ops.cast_to_input(self.position_embedding.weight, embeds)
