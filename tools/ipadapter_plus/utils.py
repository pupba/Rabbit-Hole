...
from cores.clip_vision import clip_preprocess, Output
import cores.utils
...
def get_clipvision_file(preset):
    preset = preset.lower()
    clipvision_list = cores.path_utils.get_filename_list("clip_vision")
    ...
    clipvision_file = (
        cores.path_utils.get_full_path("clip_vision", clipvision_file[0])
        if clipvision_file
        else None
    )
...
def get_ipadapter_file(preset, is_sdxl):
    preset = preset.lower()
    ipadapter_list = cores.path_utils.get_filename_list("ipadapter")
    ...
    ipadapter_file = (
        cores.path_utils.get_full_path("ipadapter", ipadapter_file[0])
        if ipadapter_file
        else None
    )
...
def get_lora_file(pattern):
    lora_list = cores.path_utils.get_filename_list("loras")
    lora_file = [e for e in lora_list if re.search(pattern, e, re.IGNORECASE)]
    lora_file = (
        cores.path_utils.get_full_path("loras", lora_file[0]) if lora_file else None
    )
...
def ipadapter_model_loader(file):
    model = cores.utils.load_torch_file(file, safe_load=True)
...
def insightface_loader(provider, model_name="buffalo_l"):
    ...
    path = os.path.join(cores.path_utils.models_dir, "insightface")
    model = FaceAnalysis(
        name=model_name,
        root=path,
        providers=[
...
def encode_image_masked_(
    ...
    model_management_utils.load_model_gpu(clip_vision.patcher)
    outputs = Output()
    ...
    for img in image_batch:
        ...
        if not hasattr(outputs, "last_hidden_state"):
            outputs["last_hidden_state"] = out[0].to(
                model_management_utils.intermediate_device()
            )
            ...
            outputs["last_hidden_state"] = torch.cat(
                (
                    ...
                    out[0].to(model_management_utils.intermediate_device()),
                ),
                dim=0,
            )
            outputs["image_embeds"] = torch.cat(
                (
                    ...
                    out[2].to(model_management_utils.intermediate_device()),
                ),
                dim=0,
            )
            outputs["penultimate_hidden_states"] = torch.cat(
                (
                    ...
                    out[1].to(model_management_utils.intermediate_device()),
                ),
                dim=0,
            )
