...
from cores.cli_args_utils import args, LatentPreviewMethod
from cores.taesd.taesd import TAESD
...
def preview_to_image(latent_image):
    ...
    if cores.model_management_utils.directml_enabled:
        latents_ubyte = latents_ubyte.to(dtype=torch.uint8)
    latents_ubyte = latents_ubyte.to(
        device="cpu",
        dtype=torch.uint8,
        non_blocking=cores.model_management_utils.device_supports_non_blocking(
            latent_image.device
        ),
    )
    ...
    if method != LatentPreviewMethod.NoPreviews:
        ...
        if latent_format.taesd_decoder_name is not None:
            taesd_decoder_path = next(
                (
                    fn
                    for fn in cores.path_utils.get_filename_list("vae_approx")
                    if fn.startswith(latent_format.taesd_decoder_name)
                ),
                "",
            )
            taesd_decoder_path = cores.path_utils.get_full_path(
